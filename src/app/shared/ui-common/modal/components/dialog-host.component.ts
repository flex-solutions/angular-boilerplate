...
export class DialogHostComponent {
  ...
  removeDialog(component: DialogComponent, closeDelay?: number) {
    ...
    if (!component.animation) { delayMs = 5; }
  ...
  removeDialogAndParent(component: DialogComponent) {
    ...
    this.dialogs.forEach(function(value, index) {
      if (index === dialogIndex || index === dialogIndex - 1) {
        _thisRef.removeDialog(
          value,
          _thisRef.getCloseDelayForParent(value, index)
        );
      }
    });
  }
  ...
  private getCloseDelayForParent(
    ...
    if (index < this.dialogs.length - 1) {
      closeDelayParent =
        component.closeDelay === undefined
          ? component.closeDelayParent
          : component.closeDelay;
    } else {
