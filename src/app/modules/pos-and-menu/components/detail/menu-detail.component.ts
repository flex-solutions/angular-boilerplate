...
export class MenuDetailComponent extends AbstractBaseComponent implements OnInit {
    ...
    private setDetail() {
        if (!this.isShowAll) {
            this.posService.findById(this.posId).subscribe(res => {
                this.subTitle = this.translateService.translate(MessageConst.sub_title_for_pos, res.name);
            });
        } else {
            this.subTitle = this.translateService.translate(MessageConst.general_sub_title);
        }
    }
    ...
    viewTopping(menuItem: MenuItemDto) {
        this.router.navigate([`pos/menu-topping/${menuItem._id}`]);
    }
}
