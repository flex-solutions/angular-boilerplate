...
import IMenuTopping, { MenuItemDto, MenuItemTypeDto } from '../../../shared/models/menu.model';

@Injectable()
export class POSService extends AbstractRestService {
    controllerName = 'pos-and-menu';
    ...
    updateMenuITem(menuItem: MenuItemDto, isApplyAll: boolean): Observable<Response> {
        if (isApplyAll) {
    ...
    getToppings(menuItemId: string): Observable<IMenuTopping[]> {
        return this.get(`toppings/${menuItemId}`);
    }

    updateToppingOption(option: any): Observable<Response> {
        return this.patch(`topping/option`, option);
    }

    updateTopping(topping: IMenuTopping): Observable<Response> {
        return this.patch(`topping`, topping);
    }
}
