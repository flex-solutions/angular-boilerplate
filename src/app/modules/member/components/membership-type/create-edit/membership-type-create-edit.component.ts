...
@Component({
  ...
  export class MembershipTypeCreateEditComponent extends AbstractFormComponent implements OnInit {
    ...
    ngOnInit() {
      super.ngOnInit();
      this.createSuccessMsg = this.isEdit === false ? this.translateService.translate('membership-type-create-form-success')
    ...
    protected onCreateForm() {
      ...
      this.formGroup = this.formbuilder.group({
        membershipTypeCode: ['', [Validators.required, Validators.pattern(/^\S*$/)]],
    ...
    private getMembershipTypeInfoForEdit() {
      if (this.isEdit === true) {
        this.membershipTypeService.getMembershipType(this.membershipTypeId).subscribe(membershipType => {
